...
function loadAgentConfig(): AgentConfig {
  return {
    ...
    maxPositionPct: parseFloat(process.env.AGENT_MAX_POSITION_PCT ?? '0.10', 10),
    stopLossPct: parseFloat(process.env.AGENT_STOP_LOSS_PCT ?? '0.05', 10),
    takeProfitPct: parseFloat(process.env.AGENT_TAKE_PROFIT_PCT ?? '0.15', 10),
    dailyDrawdownLimitPct: parseFloat(process.env.AGENT_DAILY_DRAWDOWN_PCT ?? '0.10', 10),
    minSignalScore: parseFloat(process.env.AGENT_MIN_SIGNAL_SCORE ?? '65', 10),
    minConfidence: parseFloat(process.env.AGENT_MIN_CONFIDENCE ?? '0.6', 10),
