...
// (src/chp/canonical.ts — same digest family as the decision
// ledger). The executor refuses to move capital without a valid,
// unexpired, unreplayed receipt that hashes the exact trade args.
//
...
import type { SignalDirection } from '../utils/types';
...
export interface TradeReceiptArgs {
  token: string;
  /** The decision's direction field as carried by TradeDecision (HOLD never reaches execution). */
...
export interface IssueTradeReceiptInput {
  ...
  actor: string;
  /** Canonical target of the approval, e.g. `execute_trade:LONG:CAKE`. */
  resource: string;
  args_hash: string;
  policy_version: string;
  risk: ReceiptRisk;
