...
### Row 5 — deterministic governor over probabilistic judgment: REVERSED (wave B, verified against current state)

The trade decision path is fully probabilistic-free: three strategy engines (momentum RSI/MACD, sentiment CMC divergence, mean-reversion z-score) feed weighted orchestration into the deterministic `RiskManager` (`src/agent/RiskManager.ts`) — no LLM judgment sits in the trade path, so there is nothing for a governor to dispose of, and labeled heuristic fallbacks would be a second code path maintaining nothing. The row's own settled reversal condition (LLM calls cheap, idempotent, and retried upstream) fires vacuously for the same reason: there are no LLM calls in the path at all. Reversal first recorded in the wave-B PR #5 body; this note makes it discoverable in the README.

**Revisit trigger:** a probabilistic/LLM judgment enters the trade path (e.g. an LLM strategy juror or LLM-written rationale feeding `RiskManager`). Then adopt the governor, labeled-fallback, and recorded-dissent discipline for that surface — unless those LLM calls are cheap, idempotent, and retried upstream, in which case the row's own reversal condition holds again.
